...
pub fn parse_uid(value: &Value) -> Option<String> {
    value["user"]["uid"].as_str().map(String::from)
}
...
impl<'a> GateKeeperMemberListener<'a> {
    ...
    server_token: config.gk_server_token.clone(),
    ...
    pub fn fetch_user(&self, key: String) -> Result<Value, FetchError> {
            ...
            .send()
        {
            Ok(res) => match res.status() {
                            StatusCode::OK => {
            let text = res.text().map_err(|_| FetchError::ParseError)?;
            let parsed: Value = serde_json::from_str(&text).map_err(|_| FetchError::ParseError)?;
            eprintln!("fetched uid: {}", parse_uid(&parsed).unwrap_or_else(|| "none".to_string()));
            Ok(parsed)
